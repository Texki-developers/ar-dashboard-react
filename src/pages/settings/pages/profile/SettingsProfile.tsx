...
const SettingsProfile = () => {
  ...
  const onCloseUpdateVendor = () => setShowUpdateVendor(false);
  return (
    <div>
          ...
          <div className="grid gap-2 mt-5">
            {profileSettingsConfig.map((config) => (
              <div
                ...
                <span>
                  {config?.render
                    ? config.render(data?.data?.[config.sourceKey as keyof IProfileSettings])
                    : data?.data?.[config.sourceKey as keyof IProfileSettings]}
                </span>
              </div>
            ))}
          </div>
